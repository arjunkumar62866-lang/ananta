...
// Calculate Total Ranking Income for header stat
$table = "tbl_transaction";
$ranking_income_total = incometotalnew($pdo, $table, $userid, 'Ranking Income Payout');
$ranking_income_total = round((float)($ranking_income_total ?? 0), 2);
?>

<style>
/* =========================================================
   ANANTA FINTECH THEME - RANKING INCOME REDESIGN
...
/* Header Banner */
.income-header-card {
    background: linear-gradient(135deg, rgba(234, 179, 8, 0.12) 0%, rgba(2, 132, 199, 0.10) 100%), #ffffff !important;
                                ...
                                <div class="d-flex align-items-center gap-3">
                                    ...
                                    <div>
                                        <div class="d-flex align-items-center gap-2 mb-1">
                                            <span class="badge" style="background: rgba(234, 179, 8, 0.18); color: #854d0e; font-size: 11px; font-weight: 700; border-radius: 100px; padding: 4px 12px; letter-spacing: 0.5px;">VIP RANKING REWARDS</span>
                                            <span style="font-size: 12px; color: #64748b; font-weight: 600;">RANK ACHIEVEMENTS</span>
                                        </div>
                                        <h4 class="mb-0" style="font-size: 22px; font-weight: 800; color: #0f172a;">
                                            Ranking <span style="background: linear-gradient(135deg, #eab308 0%, #ca8a04 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">Income Payouts</span> 🏆
                                        </h4>
                                        <p class="mb-0 text-muted" style="font-size: 13.5px; margin-top: 3px;">
                                            Historical log of rank achievement bonuses and VIP club reward deposits.
                                        </p>
                                    </div>
                                </div>
                                <div>
                                    <div class="px-3 py-2" style="background: #ffffff; border-radius: 14px; border: 1px solid rgba(234, 179, 8, 0.25); box-shadow: 0 4px 12px rgba(15, 23, 42, 0.04);">
                                        <span class="text-muted d-block" style="font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Total Ranking Income</span>
                                        <span class="font-weight-bold" style="font-size: 18px; color: #ca8a04; font-weight: 800;"><?php echo $hmcurrency . " " . number_format($ranking_income_total, 2); ?></span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="ananta-fintech-card">
                            ...
                            <div class="card-header-bar">
                                <div class="card-header-title">
                                    <h4><i class="fa fa-award text-warning me-2"></i> Ranking Income Log</h4>
                                    <p>Real-time record of rank reward payouts</p>
                                </div>
        ...
        $(document).ready(function () {
            let table = $('#usersTable').DataTable({
                ...
                columns: [
                    ...
                    { 
                        data: 'subject',
                        render: (data) => '<span style="font-weight: 600; color: #1e293b;">' + (data || 'Ranking Income') + '</span>'
                    },
                    { 
                        data: 'type',
                    ...
                    { 
                        data: 'created_date',
                        render: (data) => '<span style="color: #64748b; font-size: 13px;"><i class="fa fa-calendar-o me-1 text-warning"></i>' + (data || 'N/A') + '</span>'
                    },
                    { 
                        data: 'time',
                ...
                buttons: [
                    {
                        extend: 'excelHtml5',
                        title: 'Ranking_Income_Statement'
                    }
                ],
                language: {
                    search: "_INPUT_",
                    searchPlaceholder: "Search ranking income...",
