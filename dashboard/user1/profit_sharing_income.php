...
<style>
/* =========================================================
   ANANTA FINTECH THEME - PROFIT SHARING INCOME REDESIGN
...
/* Header Banner */
.income-header-card {
    background: linear-gradient(135deg, rgba(13, 148, 136, 0.10) 0%, rgba(2, 132, 199, 0.10) 100%), #ffffff !important;
                ...
                <div class="mb-3">
                    <nav aria-label="breadcrumb">
                        <div style="display: inline-flex; flex-direction: row; align-items: center; gap: 8px; padding: 6px 14px; background: #f1f5f9; border: 1px solid #e2e8f0; border-radius: 10px; font-size: 13.5px; font-weight: 600; white-space: nowrap;">
                            ...
                            <span style="color: #94a3b8; font-weight: 400;">/</span>
                            <span style="color: #0f172a; font-weight: 700;">2. Profit Sharing Income</span>
                        </div>
                    </nav>
                </div>
                                ...
                                <div class="d-flex align-items-center gap-3">
                                    ...
                                    <div>
                                        <div class="d-flex align-items-center gap-2 mb-1">
                                            <span class="badge" style="background: rgba(13, 148, 136, 0.15); color: #0d9488; font-size: 11px; font-weight: 700; border-radius: 100px; padding: 4px 12px; letter-spacing: 0.5px;">PROFIT SHARING</span>
                                            <span style="font-size: 12px; color: #64748b; font-weight: 600;">MONTHLY LEVEL EARNINGS</span>
                                        </div>
                                        <h4 class="mb-0" style="font-size: 22px; font-weight: 800; color: #0f172a;">
                                            Profit Sharing <span style="background: linear-gradient(135deg, #0d9488 0%, #0284c7 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">Income</span> 📊
                                        </h4>
                                        <p class="mb-0 text-muted" style="font-size: 13.5px; margin-top: 3px;">
                                            Breakdown of monthly level profit sharing deposits and closing period earnings.
                                        </p>
                                    </div>
                                </div>
                                <div>
                                    <div class="px-3 py-2" style="background: #ffffff; border-radius: 14px; border: 1px solid rgba(13, 148, 136, 0.2); box-shadow: 0 4px 12px rgba(15, 23, 42, 0.04);">
                                        <span class="text-muted d-block" style="font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Total Profit Sharing</span>
                                        <span class="font-weight-bold" style="font-size: 18px; color: #0d9488; font-weight: 800;"><?php echo $hmcurrency . " " . number_format($profit_sharing_total, 2); ?></span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="ananta-fintech-card">
                            ...
                            <div class="card-header-bar">
                                <div class="card-header-title">
                                    <h4><i class="fa fa-list-alt text-info me-2"></i> Profit Sharing Log</h4>
                                    <p>Real-time log of profit sharing payouts</p>
                                </div>
    ...
    <script>
        $(document).ready(function () {
            let table = $('#usersTable').DataTable({
                ajax: {
                    url: 'get_profit_sharing_income.php',
                    type: 'GET',
                    data: { type: '' },
                    dataSrc: ''
                },
                columns: [
                    ...
                    { 
                        data: 'level_num',
                        render: (data) => '<span class="level-pill"><i class="fa fa-layer-group me-1"></i>' + (data ? 'Level ' + data : 'N/A') + '</span>'
                    },
                    { 
                        data: 'closing_month',
                        render: (data) => '<span class="month-pill"><i class="fa fa-calendar me-1"></i>' + (data || 'N/A') + '</span>'
                    },
                    { 
                        data: 'subject',
                        render: (data) => '<span style="font-weight: 600; color: #1e293b;">' + (data || 'Profit Sharing Income') + '</span>'
                    },
                    { 
                        data: 'type',
                    ...
                    { 
                        data: 'created_date',
                        render: (data) => '<span style="color: #64748b; font-size: 13px;"><i class="fa fa-calendar-o me-1 text-info"></i>' + (data || 'N/A') + '</span>'
                    },
                    { 
                        data: 'time',
                ...
                buttons: [
                    {
                        extend: 'excelHtml5',
                        title: 'Profit_Sharing_Income_Statement'
                    }
                ],
                language: {
                    search: "_INPUT_",
                    searchPlaceholder: "Search profit sharing...",
