        ...
        if($error1 == 0) {
            ...
            for($i=0; $i<5; $i++) {
                ...
                if($pinfinal !== '1290') {
                    ...
                    if($isidactive == 1) {
                        if($i==0) {
                            $per_amount=getpercent($packageid,$level1); 
                        } elseif($i==1) {
                            $per_amount=getpercent($packageid,$level2);
                        } elseif($i==2) {
                            $per_amount=getpercent($packageid,$level3);
                        } elseif($i==3) {
                            $per_amount=getpercent($packageid,$level4);
                        } elseif($i==4) {
                            $per_amount=getpercent($packageid,$level5);
                        } elseif($i==5) {
                            $per_amount=getpercent($packageid,$level6);
                        } elseif($i==6) {
                            $per_amount=getpercent($packageid,$level7);
                        }

                        $transactionamount=$per_amount;
                        ...
                        if(isset($transactionamount)) {
                            $level = $i+1;
                            $messagenew="Team Building Income on Level-$level.of Id($activateuserid)";
                            insert_transction($spcode1,$transactionamount,$messagenew,$time,'CREDIT');
                            insert_userlevel($mysponserid,$activateuserid,$level);
                        }
                    } else {
                        $pinfinal =$spcode1;
                    }
            ...
            echo "<script>alert('Your Id Activate Successfully');window.location.assign('user-activate.php');</script>";

        } // end error1
    } else {
        echo "<script>alert('Your Id Activated Already');window.location.assign('user-activate');</script>";
    }
}
