...
function rank_reward($userid)
{
    ...
    // Existing values
    $current_rank = $user['rank'];   // stores text value like Manager, Gold, etc.

    // Step 2: Get left & right directs
    $left = getmydirectidleft($userid, 'Left');
    $right = getmydirectidright($userid, 'Right');
    ...
    foreach ($levels as $lvl) {
        ...
        if ($left >= $req && $right >= $req) {
            ...
            $stmtu = $pdo->prepare("
                UPDATE user 
                SET amount = amount + :amt, total_inc = total_inc + :amt, rank = :rk 
                WHERE userid = :uid
            ");
